...
capture <ip>  ─→  read guisettings.xml (resolution/overscan) → devices.yml
                                    │
                                    ▼
              tar cf + gzip on-device  ─→  verify gzip  ─→  adb pull to staging  ─→  upload to SMB
                                                                        │
deploy <ip> [--backup dev/file.tar.gz]  ─→  download from SMB  ─→  extract
    ...
    ─→  reapply stored resolution/overscan from devices.yml
```
...
- **Per-device display calibration round-trips through `devices.yml`, not the gold source.** `capture.py` reads the device's live `guisettings.xml` (`_kodi.collect_kodi_display_settings()`: `videoscreen.resolution` setting + first `<resolutions>` block's overscan) and stores it on that device's `display` field via `DeviceStore.update_display()`; a device that's never been manually calibrated legitimately captures nothing (Kodi omits at-default settings from the file). Since `deploy.py` pushes the gold config's own `guisettings.xml` (which would otherwise clobber a device's calibration), it reapplies the stored `display` afterward via `jobs.display.patch_display_settings()` — same sed-based patch `apply-display` uses for a manual one-off. Re-run `capture` to refresh the stored value after recalibrating a device live.
