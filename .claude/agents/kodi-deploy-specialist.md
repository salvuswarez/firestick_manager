...
- `src/fire_tools/_adb.py` — `AdbClient`/`AdbKeyStore`: one cached RSA signer, one connection per job (not per command). `push_file()` scales its timeout with file size (`_MIN_TRANSFER_BYTES_PER_S`) and retries once via `reconnect()` — a stalled transfer survives a longer timeout but not a fresh socket. `free_bytes()` parses `df -k` so deploy can bail before filling the device. The old `sync_tree()` per-file push was removed in 0.1.13 (it wedged `adbd`; see the deploy notes above). Key pair lives at `~/.fire_tools/adb_keys/adbkey(.pub)` for the CLI; the HA integration (`ha-cyberpunk/custom_components/firetools/.adb_keys/`) has its own separate key pair — a device only trusts a key once it's been authorized on-screen (or the two identities are reconciled by copying the key files across).
