...
- `src/fire_tools/jobs/capture.py` — `run_capture()`: ADB-connect, `collect_kodi_metadata()`, prune `PRE_CAPTURE_PRUNE_PATHS` on-device, `tar cf` + a separate `gzip` pass (not `tar czf` — toybox's `-z` integration silently truncated archives on a real device), `_verify_gzip()` the pulled archive before trusting it, `adb pull` to per-job staging, upload to SMB via `SmbClient`, write a `.meta.json` sidecar (`BackupMeta`).
- `src/fire_tools/jobs/build.py` — `run_build()`: resolve a source capture (explicit `--source` or latest under `gold/`), download + extract, then run **every** profile transform once: `prune_addons()`, `apply_setting_overrides()`/`remove_thumbnail_path_substitution()`, `apply_hub_layout()`, `apply_view_type_overrides()`. Repacks **flat** via `_pack_flat()` (`addons`/`userdata`/`media` at the tar root, no `.kodi/` wrapper) and publishes to `builds/` on SMB with the source's `BackupMeta` carried forward. `PROFILE_FOLDERS` here is the shared definition of what a profile consists of — deploy imports it.
- `src/fire_tools/jobs/deploy.py` — `run_deploy()`: check device online, resolve a build (`_resolve_build_ref()`; rejects any ref outside `builds/`), download it, install base APK if the version differs (`base_apk_local` lets a batch caller pre-resolve once, see `resolve_base_apk()`), check free space, force-stop Kodi, push the archive as **one file**, extract on-device (`_extract_on_device()`: `gzip -d` then plain `tar xf`, replacing `addons`/`userdata`/`media`), verify via `_verify_extracted()`, then `_apply_device_overrides()` — the device's stored `display` calibration plus its `settings` block from `devices.yml`. **No profile shaping happens here**; an invalid per-device value is logged and skipped, not fatal, since the profile is already on the device by that point.
...
- `src/fire_tools/_adb.py` — `AdbClient`/`AdbKeyStore`: one cached RSA signer, one connection per job (not per command). **`push_file()` uploads over a `toybox nc` stream, not `adb_shell.push()`** — that library call transfers zero bytes to these devices for anything over a few MB (see `gotcha_adb_shell_push_broken` memory). `_NcListener` holds the device-side listener open on its own connection in a worker thread, since a backgrounded `nc` dies when `adb_shell` closes the shell stream. Every push is md5-verified; `nc` drops its buffered tail if the sender closes too early. `shell()`/`pull()` still use `adb_shell` and are unaffected. `free_bytes()` parses `df -k` so deploy can bail before filling the device. The old `sync_tree()` per-file push was removed in 0.1.13. Key pair lives at `~/.fire_tools/adb_keys/adbkey(.pub)` for the CLI; the HA integration (`ha-cyberpunk/custom_components/firetools/.adb_keys/`) has its own separate key pair — a device only trusts a key once it's been authorized on-screen (or the two identities are reconciled by copying the key files across).
- `src/fire_tools/_kodi.py` — `check_device_online()`, `collect_kodi_metadata()` (kodi/android/arctic-fuse version probes; checks `skin.arctic.fuse.3` first, then `.2`/unversioned as fallback), `parse_display_settings()` (pure parser for resolution_index/overscan, used by `Scanner._probe_adb` during a scan) and `collect_kodi_display_settings()` (same, for callers holding an `AdbClient`).
...
- Capture never prunes addons — only on-device cache/thumbnail junk (`PRE_CAPTURE_PRUNE_PATHS`) before tarring. Addon pruning happens at **build** time (`prune_addons()`), on the extracted local copy, before repacking — a capture is always a faithful full snapshot.
- **Profile shaping belongs in `jobs/build.py`, never `jobs/deploy.py`.** Deploy's only per-device work is the base-APK version check and `_apply_device_overrides()` (stored `display` + `Device.settings`). If something must vary per stick, it goes in `devices.yml` under `settings`, not into deploy as a code branch.
